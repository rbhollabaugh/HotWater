...
/* This may initiate a 'read time slot' and return the status of the last command (0x44) */
...
boolean ConversionStatus(OneWire *dsp)
{
    return(dsp->read_bit());
}
...
boolean GetTemp(OneWire *dsp, byte *addr, float *retnTemp)
{
    ...
    present = dsp->reset();
    if(present)  /* Continue if sensor is actually there and responding. */
    {
        dsp->select(addr);    

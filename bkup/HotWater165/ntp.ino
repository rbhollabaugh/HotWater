...
void sendNTPpacket()
{
    ...
    for(x=0; x<NUM_IP_ADDRESS_BYTES; x++)
        ip[x] = GetEEact(NTP_IP_ADDRESS1_LOC+x);
        
    IPAddress ntpip(ip[0], ip[1], ip[2], ip[3]);
...
void AdjustMillisPerSec(time_t NewUnixTime)
{
	float time_diff, time_stamp_diff;
	float adjmillis;
return;
	if(UnixTimeTimeStamp > 0UL)
	{
		time_diff = NewUnixTime - UnixTime;
		time_stamp_diff = abs(NewUnixTime - UnixTimeTimeStamp);
		adjmillis = ((time_diff/time_stamp_diff)*10.0) * 1000.0;
		MillisPerSec = MillisPerSec-adjmillis;
	}

	UnixTimeTimeStamp = NewUnixTime;
}

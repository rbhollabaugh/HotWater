...
void ConfigParam(byte level, byte pin)
{
    ...
    if(level == 3 && pin == SAVEBUTTONPIN)
    {
        ...
        if(offset == NTP_TIME_FLAG_LOC)
            StartWiznet();
        GLCD.CursorTo(15, 7);GLCD.print("Saved");
    }
}
...
void Manual(byte level, byte pin)
{
    ...
    GLCD.print(o.desc); GLCD.EraseTextLine();
    GLCD.EraseTextLine(7);
    if(level == 2)
    {
        if(pin == SAVEBUTTONPIN)
...
void SetupForNewScreen()
{
    ...
    if(CurScreenIdx == 0)
    {
        GLCD.DrawRoundRect(GARAGE_X_POS, ZONE_Y_POS, GARAGE_WIDTH, ZONE_HEIGHT, 4);  /* Garage */
        ...
        GLCD.DrawRoundRect(HOUSE_X_POS, ZONE_Y_POS, HOUSE_WIDTH, ZONE_HEIGHT, 4);  /* House */
    ...
    if(CurScreenIdx > 0)
    {
        ...
        GLCD.print(Screens[CurScreenIdx].ScreenName);
    }
}
...
void MainScreen(byte level, byte pin)
{
    ...
    GLCD.print(dtostrf(TSdata[T1TOP_LOC].avgtemp, 5, 1, strtemp));
    GLCD.CursorToXY(TANK1_X_POS+2, 29); /* x y */
    GLCD.print(dtostrf(TSdata[T1BOT_LOC].avgtemp, 5, 1, strtemp));
    
    /* Tank2 temps */
    GLCD.CursorToXY(TANK2_X_POS+2, 13); /* x y */
    GLCD.print(dtostrf(TSdata[T2TOP_LOC].avgtemp, 5, 1, strtemp));
    GLCD.CursorToXY(TANK2_X_POS+2, 29); /* x y */
    GLCD.print(dtostrf(TSdata[T2BOT_LOC].avgtemp, 5, 1, strtemp));

    /* Collector */
    GLCD.CursorToXY(COLLECTOR_X_POS+2, 13); /* x y */
    GLCD.print(dtostrf(TSdata[COLLECTOR_LOC].avgtemp, 5, 1, strtemp));
    ...
    GLCD.print(dtostrf(TurnOnTemp, 5, 1, strtemp));
...
void Information(byte level, byte pin)
{
    MenuTimerCnt = MENU_ON_SECONDS;
    GLCD.EraseTextLine(6);
    GLCD.Printf("Ard%d %d mem%d", ARDUINO, TEENSY_VERSION, freeMemory());
    GLCD.EraseTextLine(7);
    GLCD.Printf("%s %s", __DATE__, __TIME__);
}
...
/* cnt = 3 = 3*.8 = 2.4 secs */
void AddMsg(const char *line1, const char *line2)
{
    ...
    if(!found)
    {
        ...
        Msg[idx].line1[MSG_LENGTH] = (char)NULL;
        Msg[idx].line2[MSG_LENGTH] = (char)NULL;
        Msg[idx].cnt = 3;
    }
}
...
void DelMsg()
{
    ...
    if(Msg[0].cnt == 0)
    {
        ...
        Msg[NUM_MSG-1].line1[0] = (char)NULL;
        Msg[NUM_MSG-1].line2[0] = (char)NULL;
        Msg[NUM_MSG-1].cnt = 0;
    }
}
